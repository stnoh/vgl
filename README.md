...
1. まず，Visual Studio をインストールする．VS 2026でも構わないが，VS 2022程度で十分であろう．  
  VS 2022については，以下の公式ページへのリンクを辿り，ダウンロード可能である:  
  [https://aka.ms/vs/17/release/vs_community.exe](https://aka.ms/vs/17/release/vs_community.exe)  
  もちろん，Visual Studio 2026 を使っても問題なく動作することは確認したものの，  
  このライブラリをビルドするためにVS2026をインストールするメリットはあまりなさそう．  
...
## ビルド: Windows + Visual C++

このライブラリは基本的に Visual C++ (VS 2019-2026) on Windows を想定して作られてあるものの，  
...
因みに，Ubuntu 22.04 でのビルド成功も確認したが，今後もしばらくの開発は Windows + VS2022 で行う予定であり，  
...
コンパイルできることは確認している．しかし，インストール容量が大きく，メリットが少ないと判断し推奨しない．  

### C++コードの書き方

本コードはC++11以後のModern C++やModern GLではなく，C++03のような古い書き方とLegacy GLを多く採用している．C++やGLの規格は，何れも2000年代から2010年代中盤まで多くの変革を経ており，書き方がコロコロ変わっていた時期であった．そこで，インターネット上の記事を探しても複数のやり方が混在しているため，参照に注意が必要である．このレポジトリも，Modern C++はあまり多く使っておらず，C with Classes(<=C++98)からLegacy C++(~=C++03)の書き方に留まっている．  

概ねの方針を整理すると，以下のようになる:  
